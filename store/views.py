...
def search(request):
    ...
    if request.method == "POST":
        ...
        # test for null
        if not search:
            messages.success(request , "sorry the product does not exist...")
            return render(request, "search.html", {})
        else:
            return render(request, "search.html", {'searched':search})
...
def update_password(request):
    if request.user.is_authenticated:
        ...
        messages.success(request , "you must be logged in to the page ")
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if saved_cart:
                ...
                #Get the cart
                cart = Cart(request)
                # Loop thru th cart and add the items from the data
